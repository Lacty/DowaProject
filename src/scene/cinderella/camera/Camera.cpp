...
namespace dowa {
	...
	Camera::Camera(float fov, float _near, float _far) :Camera() {
	...
	void Camera::bound() {
		...
		ci::app::console() << "ViewL :" << mViewLeft << std::endl;
		ci::app::console() << "ViewR :" << mViewRight << std::endl;
		ci::app::console() << "ViewT :" << mViewTop << std::endl;
		ci::app::console() << "ViewB :" << mViewBottom << std::endl;

		// 描画範囲がステージ外に出ないようにする
		if (topLeft.x < TopLeft.x) {
			float offset = TopLeft.x - topLeft.x;
			ci::app::console() << "offset :" << offset << std::endl;
			mPos.x += offset;
			//mCamera.setEyePoint(mPos);
			return;
		}
